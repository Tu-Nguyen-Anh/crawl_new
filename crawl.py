...
def get_category_info(category_code, categories_collection):
...
def crawl_vnexpress_category(category_url, category_code, collection, categories_collection):
    ...
    try:
        ...
        category_info = get_category_info(category_code, categories_collection)
        ...
        for article in articles:
            ...
            description = (article_soup.find('p', class_='description') or '').text.strip()
            content = (article_soup.find('article', class_='fck_detail') or '').get_text(separator='\n', strip=True)
            ...
            if date_tag:
                try:
                    ...
                    logger.warning(f"Không thể parse ngày: {date_tag.text.strip()}, lỗi: {e}")
            ...
            logger.info(f"Đã lưu: {title} - Tác giả: {author}")
...
def crawl_tienphong_category(category_url, category_code, collection, categories_collection):
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
    try:
        response = requests.get(category_url, headers=headers)
        response.raise_for_status()
        ...
        for article in articles:
            ...
            author = None
            if author_div := article_soup.find('div', class_='article__author'):
                author = (author_div.find('span', class_='name cms-author') or '').text.strip()
            ...
            if not author:
                for script in article_soup.find_all('script', type='application/ld+json'):
                    try:
                        if (json_data := json.loads(script.text)).get('@type') == 'NewsArticle' and json_data.get('author'):
...
def crawl_all_sources(articles_collection, categories_collection):
    ...
    for base_url, categories, crawl_func in [
        ('https://vnexpress.net/', VNEXPRESS_CATEGORIES, crawl_vnexpress_category),
        ('https://nhandan.vn/', NHANDAN_CATEGORIES, crawl_nhandan_category),
        ('https://tienphong.vn/', TIENPHONG_CATEGORIES, crawl_tienphong_category)
    ]:
        for code in categories:
            crawl_func(f"{base_url}{code}", code, articles_collection, categories_collection)
